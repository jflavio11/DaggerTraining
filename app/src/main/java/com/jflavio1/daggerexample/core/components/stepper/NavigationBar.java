...
//            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < navigationTabs.size(); i++)
            navigationTabs.get(i).onDrawBackground(canvas, p);
        drawSpring(canvas);
        for (int i = 0; i < navigationTabs.size(); i++)
...
//        int progress = (int) (length * INDICATOR_ANIM_DURATION);
//        seek(progress);
//      int xc= (int) NavigationTabs.get(position).getPoint().getX();
...
//        int clickPosition = (int) (NavigationTabs.get(position).getPoint().getX() + tabRadius);
    ...
    public boolean setCurrentPosition(int currentPosition) {
        ...
        try {
            if (navigationTabs != null && currentPosition < navigationTabs.size()) {
                ...
                //onTabSelected.onTabSelected(this.currentPosition, NavigationTabs.get(NavigationTabs.size() - 2), NavigationTabs.get(NavigationTabs.size() - 1));
                return false;
            }
